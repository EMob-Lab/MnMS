...
if __name__ == '__main__':
    ...
    start = datetime.strptime(df_vehs['TIME'].iloc[0], "%H:%M:%S.%f")
    end = datetime.strptime(df_vehs['TIME'].iloc[-1], "%H:%M:%S.%f")
    simulation_duration = (end - start).total_seconds()

    diff = df_vehs['TIME'] != df_vehs['TIME'].shift()
    diff.iloc[0] = False
    flow_dt = datetime.strptime(df_vehs.loc[diff.idxmax()]['TIME'], "%H:%M:%S.%f") - start
    ...
    # Timer
    time_template = "%H:%M:%S"
    time_text = ax1.text(0.4, -0.05, start.strftime(time_template), transform=ax1.transAxes, fontsize=18)
    time_text.set_bbox(dict(facecolor='grey', alpha=0.5))
    time_text.set_text(start.strftime(time_template))
    ...
    # Vehicles
    scat_vehs = ax1.scatter([], [], c='blue', s=30, marker='h',alpha=0.8)
